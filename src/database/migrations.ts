...
async function applySchemaV13(db: SQLiteDatabase): Promise<void> {
  await db.execAsync(`
    ALTER TABLE daily_stock_entries ADD COLUMN movement_reason TEXT;
  `).catch(() => {});
}
...
async function applySchemaV15(db: SQLiteDatabase): Promise<void> {
  await db.execAsync(`
    ALTER TABLE stock_items ADD COLUMN max_quantity REAL;
  `).catch(() => {});
}
